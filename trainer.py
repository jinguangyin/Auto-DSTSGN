...
class Trainer():
    ...
    def train_weight(self, input, real_val, ycl, batches_seen = None, mode = Mode.TWO_PATHS):
        ...
        if self.cl:
            output = self.model(input, ycl = ycl, batches_seen = self.iter, task_level = self.task_level, mode = Mode.TWO_PATHS)
        else:
            output = self.model(input, ycl = ycl, batches_seen = self.iter, task_level = self.seq_out_len, mode = Mode.TWO_PATHS)
